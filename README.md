...
Both REPLs additionally accept `--session-memory` (enable the session-memory-dream M3 incremental fold; default off, mirrors `--extract-memories`). When enabled, `maybe_update_session_memory` runs at every stop hook to keep `SessionMemoryState` warm across turns; at compaction time `_force_compact` injects `SessionMemorySummarizer(state)` so a WARM reuse adds **zero** summarization provider calls (cold/empty state falls through to the configured Rule/LLM summarizer — null-vs-throw contract). `/stats` surfaces `sm_compact_reuses` / `sm_compact_misses` so the saving is countable, and the existing `compact` trace channel emits `reused=<bool>` (channel vocab stays frozen at 11 names). `--dream-on-exit` (default off) fires one `DreamConsolidator` run at REPL `/exit` / EOF / max-turns shutdown — gates are bypassed so short sessions still consolidate; requires `--memory-dir` (or `SIMPLE_AGENT_MEMORY_DIR`); uses the deterministic Jaccard path under `MockProvider` and the LLM path under real providers.

The `simple-agent memory` subcommand family (`add` / `list` / `delete` / `search` / `show` / `update` / `migrate-format`) gains a `dream` subcommand (session-memory-dream M7). Dry-run by default — copies the memory dir to a `TemporaryDirectory`, runs `DreamConsolidator.consolidate()` with all gates bypassed on the copy, then discards it (the real dir is byte-identical). `--apply` runs on the real dir under the M5 gate cascade (24 h time gate, ≥5 sessions, 10-min scan throttle, PID mutex via `.consolidate-lock`). `--apply --force` zeroes all thresholds for demo. `--provider openai` constructs `OpenAIProvider` (defaults to `qwen-plus-latest`, overridable via `OPENAI_MODEL`; reads `OPENAI_API_KEY` / `DASHSCOPE_API_KEY`) for the LLM consolidation path; without it, the deterministic Jaccard / mtime fallback runs. Exit codes: `0` = success or gate-closed no-op or dry-run, `1` = consolidation exception, `2` = bad `memory_dir`. See `docs/DECISIONS/0005-dream-cli-no-cron-divergence.md` for the no-cron rationale.
