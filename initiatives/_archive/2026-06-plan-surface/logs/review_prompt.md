...
The script substitutes plan-surface, plan-srf,
2026-06-plan-surface, and 17e616dc7feeb563019ec3ba1b0fbd421b21554e (4 tokens) before piping
this file as stdin.
...
# Phase 2B + 2C — Multi-agent review + wrap-up for `plan-surface`

You are the **MAIN REVIEW AGENT** for the `plan-surface`
...
3. **Phase 2C wrap-up**: create `REVIEW.md` and
   ...
   commit the final archive result as `[plan-srf/wrap]`.
4. **Final exit-gate verification**: ensure tests, archive/current state,
   ...
   final HEAD should be the `[plan-srf/wrap]` commit.
...
```bash
git -C python-replica log 17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD --oneline | grep -F "[plan-srf/M{N}]"
```

This must return at least one match. The `17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD`
...
`commit_prefix`. `17e616dc7feeb563019ec3ba1b0fbd421b21554e` was substituted by the shell
...
* number of commits in `17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD`
...
`description`: `"code-review-for-plan-surface"`

----- BEGIN PROMPT[code-reviewer] -----

You are the **code-reviewer subagent** for the `plan-surface`
...
7. `git -C python-replica log 17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD --oneline`
8. `git -C python-replica diff 17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD -- src/ tests/`
...
| Dimension                   | What to look for                                                                |
| --------------------------- | ------------------------------------------------------------------------------- |
| Commit hygiene              | Subject matches `[plan-srf/M{N}]`; body explains why.                  |
...
`description`: `"doc-curator-candidates-for-plan-surface"`

----- BEGIN PROMPT[doc-curator-candidate-finder] -----

You are the **doc-curator-candidate-finder subagent** for the
`plan-surface` initiative. You identify which project docs may
...
2. `git -C python-replica diff 17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD --stat -- src/ pyproject.toml`
3. `git -C python-replica diff 17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD --name-only -- src/ pyproject.toml`
4. `git -C python-replica diff 17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD -- src/ pyproject.toml`
...
`[plan-srf/review-fix] <short description>`

Review-time documentation-only fixes made before Phase 2C wrap-up must be
committed as:

`[plan-srf/review-doc] <short description>`
...
final `[plan-srf/wrap]` commit should archive and summarize the
...
```bash
git -C python-replica diff 17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD --stat -- src/ pyproject.toml
git -C python-replica diff 17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD --name-only -- src/ pyproject.toml
````
...
- `subagent_type` = `"general-purpose"`
- `description` = `"owner-brief-narrator-for-plan-surface"`
...
You are the **demo-narrator subagent** for the `plan-surface`
...
4. `git -C python-replica log 17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD --oneline`
5. `git -C python-replica diff 17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD -- src/ examples/ README.md tests/`
...
| 项        | 之前（baseline `17e616dc7feeb563019ec3ba1b0fbd421b21554e`） | 之后（本 initiative 结束） |
| -------- | ---------------------------------- | ------------------- |
| <功能维度 1> | <旧行为>                              | <新行为>               |
| <功能维度 2> | <旧行为>                              | <新行为>               |
...
- `<finding title>` — Tier <A/B/C> — severity <HIGH/MEDIUM/LOW>
  ...
  - Fix commit: `<sha> [plan-srf/review-fix or review-doc] ...`
  - Files changed: `<paths>`
...
本次 initiative 在 `17e616dc7feeb563019ec3ba1b0fbd421b21554e..HEAD` 范围共 <N> 个 commit，
...
```bash
cd python-replica
git mv initiatives/current initiatives/_archive/2026-06-plan-surface
...
```text
initiatives/_archive/2026-06-plan-surface/logs/review.log
```
...
- **All N milestones verified**: `[plan-srf/M1]` (`<sha>`), ...
...
- **Archive committed** as `<wrap-sha> [plan-srf/wrap]`
- **Final HEAD**: `<wrap-sha>`
...
```bash
git add initiatives/_archive/2026-06-plan-surface/logs/review.log
```
...
been committed before Phase 2C as `[plan-srf/review-fix]` or
`[plan-srf/review-doc]`. Do not squash those repair commits into
...
git commit -m "[plan-srf/wrap] post-execution review + archive (review-and-repair)

REVIEW.md          : initiatives/_archive/2026-06-plan-surface/REVIEW.md
OWNER_BRIEF.zh-CN  : initiatives/_archive/2026-06-plan-surface/OWNER_BRIEF.zh-CN.md
...
```bash
if ! git log --oneline -1 | grep -qF "[plan-srf/wrap]"; then
  git log --oneline -8
  echo "ERROR: final HEAD is not [plan-srf/wrap]"
  exit 1
fi
```
...
```bash
WRAP_COMMIT="$(git log --format='%H %s' | grep -F "[plan-srf/wrap]" | head -n 1 | awk '{print $1}')"
PRE_WRAP_SUBJECTS="$(git log --format='%s' 17e616dc7feeb563019ec3ba1b0fbd421b21554e.."${WRAP_COMMIT}^" 2>/dev/null || true)"
...
```text
Review complete. OWNER_BRIEF.zh-CN.md is at initiatives/_archive/2026-06-plan-surface/OWNER_BRIEF.zh-CN.md. Attach to this session via your browser extension or Claude desktop app to ask follow-up questions in any language, or /exit to end this session.
```
