...
The script substitutes session-memory-dream, sm-dream,
2026-06-session-memory-dream, and 094cf90d09fef37b3f8357b4e2b8de0434834dfd (4 tokens) before piping
this file as stdin.
...
# Phase 2B + 2C — Multi-agent review + wrap-up for `session-memory-dream`

You are the **MAIN REVIEW AGENT** for the `session-memory-dream`
...
3. **Phase 2C wrap-up**: create `REVIEW.md` and
   ...
   commit the final archive result as `[sm-dream/wrap]`.
4. **Final exit-gate verification**: ensure tests, archive/current state,
   ...
   final HEAD should be the `[sm-dream/wrap]` commit.
...
```bash
git -C python-replica log 094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD --oneline | grep -F "[sm-dream/M{N}]"
```

This must return at least one match. The `094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD`
...
`commit_prefix`. `094cf90d09fef37b3f8357b4e2b8de0434834dfd` was substituted by the shell
...
* number of commits in `094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD`
...
`description`: `"code-review-for-session-memory-dream"`

----- BEGIN PROMPT[code-reviewer] -----

You are the **code-reviewer subagent** for the `session-memory-dream`
...
7. `git -C python-replica log 094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD --oneline`
8. `git -C python-replica diff 094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD -- src/ tests/`
...
| Dimension                   | What to look for                                                                |
| --------------------------- | ------------------------------------------------------------------------------- |
| Commit hygiene              | Subject matches `[sm-dream/M{N}]`; body explains why.                  |
...
`description`: `"doc-curator-candidates-for-session-memory-dream"`

----- BEGIN PROMPT[doc-curator-candidate-finder] -----

You are the **doc-curator-candidate-finder subagent** for the
`session-memory-dream` initiative. You identify which project docs may
...
2. `git -C python-replica diff 094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD --stat -- src/ pyproject.toml`
3. `git -C python-replica diff 094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD --name-only -- src/ pyproject.toml`
4. `git -C python-replica diff 094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD -- src/ pyproject.toml`
...
`[sm-dream/review-fix] <short description>`

Review-time documentation-only fixes made before Phase 2C wrap-up must be
committed as:

`[sm-dream/review-doc] <short description>`
...
final `[sm-dream/wrap]` commit should archive and summarize the
...
```bash
git -C python-replica diff 094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD --stat -- src/ pyproject.toml
git -C python-replica diff 094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD --name-only -- src/ pyproject.toml
````
...
- `subagent_type` = `"general-purpose"`
- `description` = `"owner-brief-narrator-for-session-memory-dream"`
...
You are the **demo-narrator subagent** for the `session-memory-dream`
...
4. `git -C python-replica log 094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD --oneline`
5. `git -C python-replica diff 094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD -- src/ examples/ README.md tests/`
...
| 项        | 之前（baseline `094cf90d09fef37b3f8357b4e2b8de0434834dfd`） | 之后（本 initiative 结束） |
| -------- | ---------------------------------- | ------------------- |
| <功能维度 1> | <旧行为>                              | <新行为>               |
| <功能维度 2> | <旧行为>                              | <新行为>               |
...
- `<finding title>` — Tier <A/B/C> — severity <HIGH/MEDIUM/LOW>
  ...
  - Fix commit: `<sha> [sm-dream/review-fix or review-doc] ...`
  - Files changed: `<paths>`
...
本次 initiative 在 `094cf90d09fef37b3f8357b4e2b8de0434834dfd..HEAD` 范围共 <N> 个 commit，
...
```bash
cd python-replica
git mv initiatives/current initiatives/_archive/2026-06-session-memory-dream
...
```text
initiatives/_archive/2026-06-session-memory-dream/logs/review.log
```
...
- **All N milestones verified**: `[sm-dream/M1]` (`<sha>`), ...
...
- **Archive committed** as `<wrap-sha> [sm-dream/wrap]`
- **Final HEAD**: `<wrap-sha>`
...
```bash
git add initiatives/_archive/2026-06-session-memory-dream/logs/review.log
```
...
been committed before Phase 2C as `[sm-dream/review-fix]` or
`[sm-dream/review-doc]`. Do not squash those repair commits into
...
git commit -m "[sm-dream/wrap] post-execution review + archive (review-and-repair)

REVIEW.md          : initiatives/_archive/2026-06-session-memory-dream/REVIEW.md
OWNER_BRIEF.zh-CN  : initiatives/_archive/2026-06-session-memory-dream/OWNER_BRIEF.zh-CN.md
...
```bash
if ! git log --oneline -1 | grep -qF "[sm-dream/wrap]"; then
  git log --oneline -8
  echo "ERROR: final HEAD is not [sm-dream/wrap]"
  exit 1
fi
```
...
```bash
WRAP_COMMIT="$(git log --format='%H %s' | grep -F "[sm-dream/wrap]" | head -n 1 | awk '{print $1}')"
PRE_WRAP_SUBJECTS="$(git log --format='%s' 094cf90d09fef37b3f8357b4e2b8de0434834dfd.."${WRAP_COMMIT}^" 2>/dev/null || true)"
...
```text
Review complete. OWNER_BRIEF.zh-CN.md is at initiatives/_archive/2026-06-session-memory-dream/OWNER_BRIEF.zh-CN.md. Attach to this session via your browser extension or Claude desktop app to ask follow-up questions in any language, or /exit to end this session.
```
