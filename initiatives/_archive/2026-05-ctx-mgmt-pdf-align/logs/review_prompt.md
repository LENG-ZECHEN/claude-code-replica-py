...
The script substitutes ctx-mgmt-pdf-align, ctx-pdf,
2026-05-ctx-mgmt-pdf-align, and 8f1d98f01f764a32df538b6319115d392696ab4e (4 tokens) before piping
this file as stdin.
...
# Phase 2B + 2C — Multi-agent review + wrap-up for `ctx-mgmt-pdf-align`

You are the **MAIN REVIEW AGENT** for the `ctx-mgmt-pdf-align`
...
3. **Phase 2C wrap-up**: create `REVIEW.md` and
   ...
   commit everything as `[ctx-pdf/wrap]`.
...
```bash
git -C python-replica log 8f1d98f01f764a32df538b6319115d392696ab4e..HEAD --oneline | grep -F "[ctx-pdf/M{N}]"
```

This must return at least one match. The `8f1d98f01f764a32df538b6319115d392696ab4e..HEAD`
...
`commit_prefix`. `8f1d98f01f764a32df538b6319115d392696ab4e` was substituted by the shell
...
* number of commits in `8f1d98f01f764a32df538b6319115d392696ab4e..HEAD`
...
`description`: `"code-review-for-ctx-mgmt-pdf-align"`

----- BEGIN PROMPT[code-reviewer] -----

You are the **code-reviewer subagent** for the `ctx-mgmt-pdf-align`
...
7. `git -C python-replica log 8f1d98f01f764a32df538b6319115d392696ab4e..HEAD --oneline`
8. `git -C python-replica diff 8f1d98f01f764a32df538b6319115d392696ab4e..HEAD -- src/ tests/`
...
| Dimension                   | What to look for                                                                |
| --------------------------- | ------------------------------------------------------------------------------- |
| Commit hygiene              | Subject matches `[ctx-pdf/M{N}]`; body explains why.                  |
...
`description`: `"doc-curator-candidates-for-ctx-mgmt-pdf-align"`

----- BEGIN PROMPT[doc-curator-candidate-finder] -----

You are the **doc-curator-candidate-finder subagent** for the
`ctx-mgmt-pdf-align` initiative. You identify which project docs may
...
2. `git -C python-replica diff 8f1d98f01f764a32df538b6319115d392696ab4e..HEAD --stat -- src/ pyproject.toml`
3. `git -C python-replica diff 8f1d98f01f764a32df538b6319115d392696ab4e..HEAD --name-only -- src/ pyproject.toml`
4. `git -C python-replica diff 8f1d98f01f764a32df538b6319115d392696ab4e..HEAD -- src/ pyproject.toml`
...
```bash
git -C python-replica diff 8f1d98f01f764a32df538b6319115d392696ab4e..HEAD --stat -- src/ pyproject.toml
git -C python-replica diff 8f1d98f01f764a32df538b6319115d392696ab4e..HEAD --name-only -- src/ pyproject.toml
````
...
- `subagent_type` = `"general-purpose"`
- `description` = `"owner-brief-narrator-for-ctx-mgmt-pdf-align"`
...
You are the **demo-narrator subagent** for the `ctx-mgmt-pdf-align`
...
4. `git -C python-replica log 8f1d98f01f764a32df538b6319115d392696ab4e..HEAD --oneline`
5. `git -C python-replica diff 8f1d98f01f764a32df538b6319115d392696ab4e..HEAD -- src/ examples/ README.md tests/`
...
| 项        | 之前（baseline `8f1d98f01f764a32df538b6319115d392696ab4e`） | 之后（本 initiative 结束） |
| -------- | ---------------------------------- | ------------------- |
| <功能维度 1> | <旧行为>                              | <新行为>               |
| <功能维度 2> | <旧行为>                              | <新行为>               |
...
本次 initiative 在 `8f1d98f01f764a32df538b6319115d392696ab4e..HEAD` 范围共 <N> 个 commit，
...
```bash
cd python-replica
git mv initiatives/current initiatives/_archive/2026-05-ctx-mgmt-pdf-align
...
```text
initiatives/_archive/2026-05-ctx-mgmt-pdf-align/logs/review.log
```
...
- **All N milestones verified**: `[ctx-pdf/M1]` (`<sha>`), ...
...
- **Archive committed** as `<wrap-sha> [ctx-pdf/wrap]`
...
```bash
git add initiatives/_archive/2026-05-ctx-mgmt-pdf-align/logs/review.log
```
...
git commit -m "[ctx-pdf/wrap] post-execution review + archive (multi-agent)

REVIEW.md          : initiatives/_archive/2026-05-ctx-mgmt-pdf-align/REVIEW.md
OWNER_BRIEF.zh-CN  : initiatives/_archive/2026-05-ctx-mgmt-pdf-align/OWNER_BRIEF.zh-CN.md
...
```text
Review complete. OWNER_BRIEF.zh-CN.md is at initiatives/_archive/2026-05-ctx-mgmt-pdf-align/OWNER_BRIEF.zh-CN.md. Attach to this session via your browser extension or Claude desktop app to ask follow-up questions in any language, or /exit to end this session.
```
