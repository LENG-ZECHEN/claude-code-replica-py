...
The script substitutes {{INITIATIVE_SLUG}}, {{COMMIT_PREFIX}},
...
# Phase 2B + 2C — Multi-agent review + wrap-up for `{{INITIATIVE_SLUG}}`

You are the **MAIN REVIEW AGENT** for the `{{INITIATIVE_SLUG}}`
...
This must return at least one match. The `{{BASELINE_COMMIT}}..HEAD`
...
`commit_prefix`. `{{BASELINE_COMMIT}}` was substituted by the shell
...
* number of commits in `{{BASELINE_COMMIT}}..HEAD`
...
`description`: `"code-review-for-{{INITIATIVE_SLUG}}"`

----- BEGIN PROMPT[code-reviewer] -----

You are the **code-reviewer subagent** for the `{{INITIATIVE_SLUG}}`
...
7. `git -C python-replica log {{BASELINE_COMMIT}}..HEAD --oneline`
8. `git -C python-replica diff {{BASELINE_COMMIT}}..HEAD -- src/ tests/`
...
| Dimension                   | What to look for                                                                |
| --------------------------- | ------------------------------------------------------------------------------- |
| Commit hygiene              | Subject matches `[{{COMMIT_PREFIX}}/M{N}]`; body explains why.                  |
...
`description`: `"doc-curator-candidates-for-{{INITIATIVE_SLUG}}"`

----- BEGIN PROMPT[doc-curator-candidate-finder] -----

You are the **doc-curator-candidate-finder subagent** for the
`{{INITIATIVE_SLUG}}` initiative. You identify which project docs may
...
2. `git -C python-replica diff {{BASELINE_COMMIT}}..HEAD --stat -- src/ pyproject.toml`
3. `git -C python-replica diff {{BASELINE_COMMIT}}..HEAD --name-only -- src/ pyproject.toml`
4. `git -C python-replica diff {{BASELINE_COMMIT}}..HEAD -- src/ pyproject.toml`
...
You are the **demo-narrator subagent** for the `{{INITIATIVE_SLUG}}`
...
4. `git -C python-replica log {{BASELINE_COMMIT}}..HEAD --oneline`
5. `git -C python-replica diff {{BASELINE_COMMIT}}..HEAD -- src/ examples/ README.md tests/`
...
| 项        | 之前（baseline `{{BASELINE_COMMIT}}`） | 之后（本 initiative 结束） |
| -------- | ---------------------------------- | ------------------- |
| <功能维度 1> | <旧行为>                              | <新行为>               |
| <功能维度 2> | <旧行为>                              | <新行为>               |
...
```markdown
# OWNER BRIEF（中文）— {{INITIATIVE_SLUG}}
...
本次 initiative 在 `{{BASELINE_COMMIT}}..HEAD` 范围共 <N> 个 commit，
...
| pytest | <before> -> <after> |
| mypy + ruff | <status> |
| English review | [`REVIEW.md`](./initiatives/_archive/{{ARCHIVE_SLUG}}/REVIEW.md) |
| Owner brief 中文 | [`OWNER_BRIEF.zh-CN.md`](./initiatives/_archive/{{ARCHIVE_SLUG}}/OWNER_BRIEF.zh-CN.md) |
...
```text
initiatives/_archive/{{ARCHIVE_SLUG}}/logs/review.log
```
...
```text
Review complete. OWNER_BRIEF.zh-CN.md is at initiatives/_archive/{{ARCHIVE_SLUG}}/OWNER_BRIEF.zh-CN.md. Attach to this session via your browser extension or Claude desktop app to ask follow-up questions in any language, or /exit to end this session.
```
